...
メンバーが会議の文字起こし等をルームに添付（PDF／.docx／.txt等）またはGoogleドキュメント／
スプレッドシートのURLを貼ってメンションすると、議事録作成・要約・宿題の抽出などを行う。

- 判定はコード側で決定的に行う: メンション本文に対応形式の添付（`[download:ID]`）か
  docs.google.comのURLがあれば文書タスク。画像等の添付だけの通常質問はQ&Aのまま。
...
- PDF（2026-08-17追加）はテキスト抽出せず、原本をそのままClaudeへ渡す（documentブロック）。
  スキャン画像のPDFも読める。上限は合計15MB／50ページ（APIの32MB上限に収めるため合計で判定）。
  PDFは本文テキストを持たないため、契約書かどうかはファイル名と依頼文から判定する。
  印紙税の依頼はweb_fetchで推論が複数回まわるので、PDFにキャッシュ印を付けて再課金を避ける
- ハンドブック・プロンプトキャッシュを使わない軽量フロー（コストは文書の長さ次第。
  文字起こし3万字で1回あたり約20〜40円。PDFは実測で契約書14ページ・1.4MBが約15,000トークン
  ＝入力約7円、回答込みで1回あたり約20〜25円）
...
- FAX送付状ひな形は「汎用」シート1枚だけに削ってある。原本の取引先別15シートを残すと、
  1社あての下書きに他14社の担当者名・FAX番号が同梱されて相手先へ渡るため。共有文字列表も
  空にしないと、シートを消しただけでは文字列がファイル内に残る（139KB→14KB）
- FAX送付状の宛先は `templates/fax_destinations.json`（原本の取引先別シートから機械抽出した
  15件）を参照し、会社名が一致すればFAX番号・TEL・担当者を自動で埋める
- 「汎用」シートは会社名（D10）と敬称（H10）が別の欄。敬称込みで会社名を入れると
  「○○ 御中 御中」になるため、末尾の敬称を切り離して入れる
- 計算チェーン（calcChain.xml）はシートIDまで見て抜く。同じ座標が全シートにあるため、
  座標だけで消すと全件消える。空になる場合は[Content_Types].xmlとrelsの参照ごと外す
