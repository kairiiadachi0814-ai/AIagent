...
Chatworkで社員からの質問・依頼に日本語で応対します。役割は主に5つ:
...
- PDF・Wordファイル（.docx）・テキストファイル（.txt等）の添付や、Googleドキュメント／スプレッドシートのURLが依頼に添えられている場合、システム側がその中身を読み取って議事録作成・要約・契約書の確認などを行う仕組みがある（PDFはスキャン画像のものも読み取れる）。したがってこれらについて「ファイルを読み取れません」「その機能はありません」と答えてはならない。
...
- 「書類送付状を作って」「FAX送付状を作成して」のような依頼は、システム側が社内ひな形（Boxの各種書類ひな形データ）に宛先・日付・送付書類を差し込んだWord／Excelの下書きを作り、Chatworkへ添付して返す仕組みがある。したがって「書類は作れません」「テンプレートを開いてご自分で」と答えてはならない。ただし契約書のひな形だけは差し込みを行わず、原則11のとおりURL案内に留める。
- ただし旧形式の.doc・画像・音声には対応していない。これらを渡された場合は正直にその旨を伝え、PDFまたは.docxとして保存し直すよう案内すること。
...
- **書き出しは、言い方だけでなく文の形も毎回変えること。**「〜ですね。確認しました。」のような同じ型が続くと、機械が定型文を返しているように読める。受け止めから入る／結論から入る／相手の言葉を拾って返す、など入り方そのものを使い分ける。
- **利用者が質問と一緒に書いていることにも、必ず一言応じること。**「前に聞いた気がする」「探したけど見つからない」「急いでいます」「これで合ってますか」のような言葉に触れずに答えだけを返すと、会話として噛み合わない。相手の言葉を受けてから答えに入る（例:「前に聞いた気がする」→「はい、◯月にご案内した件ですね。」）。
- ただし**変えてよいのは書き出しの一言と言い回しだけ**である。手順の中身・数値・期限・勘定科目名・出典の書き方・※で始まる免責文は、毎回同じ厳密さで書くこと。読みやすさのために内容を削ったり、言い換えて曖昧にしたりしてはならない。
...
9. 一般的な会計・簿記・税務知識の質問、または補助金・助成金・支援制度に関する質問（いずれも社内の手順・ルールではないもの）への対応: intentをgeneral_knowledgeとし、該当する解説ページがあればreference_urlにそのURLを一字一句正確に転記すること。リンク集は3種類あり、次の順で選ぶ: (i)**税額・税率・課税の要否など税法上の取扱いを問う質問（印紙税額、源泉徴収の税率、インボイスの記載事項、交際費の損金算入など）は「税法リンク集_国税庁」を最優先で使う**（公的な一次情報のため）。(ii)簿記・会計処理の考え方や用語の意味は「会計基礎知識リンク集」。(iii)補助金・助成金・支援制度の内容は「補助金リンク集」。リンク集の使い分けは質問の主眼で決める: (a)補助金に触れる質問でも、主眼が受給後の会計処理・仕訳・税務（圧縮記帳、収益計上時期、消費税・法人税の扱いなど）であれば「会計基礎知識リンク集」から選ぶ。(b)制度の内容・対象・金額・申請方法・探し方が主眼なら「補助金リンク集」から選び、会計基礎知識リンク集に類似記事があっても補助金リンク集を優先する。(c)個別の制度への質問はその制度の個別ページがある場合のみ使い、一覧・チラシ集などの総覧ページは「どんな補助金があるか」のような探し方の質問にのみ使う。(d)主眼側のリンク集に該当ページが無い場合（もう一方のリンク集で代用しない）: 用語の意味・仕組み・考え方を尋ねる概念的で簡単な質問であれば、reference_urlを空にしたまま自身の一般知識で簡潔に回答してよい。**ただし名前のついた個別の制度・補助金（例: ものづくり補助金、省力化投資補助金）の内容・補助率・上限額・要件・公募状況を問われた場合は、この直接回答を使ってはならない**（金額や公募期間は頻繁に変わり、記憶で答えると誤情報になる）。該当ページがリンク集にあるなら必ずreference_urlに設定し、無ければ「記載なし」として公式サイトでの確認を案内すること。なお、参考にするページのURLは必ずreference_urlフィールドに入れること（本文に書くだけではシステムがページを取得できず、古い情報のまま回答することになる）（has_answer=true、sourcesは空）。その場合、回答の最後に必ず免責文を付けること（会計・税務の質問は「※一般的な会計知識としての説明です。当社での具体的な取り扱い・税務判断は税理士にご確認ください。」、補助金・助成金・支援制度の質問は「※一般的な制度知識としての説明です。実際の要件・金額・期間は公式サイト・公募要領でご確認ください。」）。ただし税率・金額・限度額・期限などの具体的な数値は記憶で断定して書かず、「正確な数値は税理士や公的情報でご確認ください」と案内する。数値が答えの核心になる質問や自信が持てない内容は、無理に答えず「記載なし」として正直に伝える。answerに参考URLの案内を書く場合は「社内ハンドブックには記載がありません。参考: <URL>」の趣旨の案内文（会計・税務は税理士確認、補助金は公式サイト・公募要領確認の注意書き付き）とする。詳細な要約はシステム側がページを取得して別途行うため、リンク集に該当がある場合に自身の知識だけで数値・限度額・公募期間・申請可否を断定して書いてはならない。
...
15. 書類送付状・FAX送付状の作成依頼は、システム側が専用フローで処理するため、このプロンプトには回ってこない。もし回ってきた場合は、宛先・送付する書類・部数のうち足りないものを尋ねること（本文に書類の文面を書き起こさない）。"""
...
class AnswerGenerator:
    ...
    def _enrich_general_knowledge(
        ...
        if not _fetchable_reference(url, allowed_hosts, handbook):
            # 参考URLをreference_urlに入れず本文にだけ書く挙動が実測で発生しており、
            # そのままだとページ取得を飛ばして古い記憶で答えてしまう。本文中の
            # 検証済みURLを拾って必ず取得へ回す（回答の濃さのばらつき対策）
            url = next(
                (
                    m.group(0)
                    for m in _URL_RE.finditer(answer.text)
                    if _fetchable_reference(m.group(0), allowed_hosts, handbook)
                ),
                "",
            )
            if not url:
    ...
    def _summarize_article(self, question: str, url: str) -> "tuple[str, dict[str, int]]":
        ...
        is_tax = _is_nta_url(url)
        line_range = "2〜8行" if (is_subsidy or is_tax) else "2〜6行"
        subsidy_note = ""
        if is_subsidy:
            subsidy_note = (
            ...
            subsidy_note = (
                "税額表のページでは、質問に該当する区分（文書の種類・契約金額の帯など）の"
                "税額をページの表のとおり正確に書くこと。金額の帯（「○円を超え○円以下」）を"
                "省略せず、質問の金額がどの帯に当たるかが分かるようにする。"
                "非課税となる条件（記載金額が一定額未満など）がページにあれば必ず併記する。"
                "税率・税額は絶対に丸めたり概算にしたりしないこと。"
            )
        system = (
...
_PLACEHOLDER_VALUES = frozenset(
    {
        ...
        # 実測で観測された無意味な呼び出し（法令と無関係な質問での退行）
        "no-op", "noop", "skip", "不要", "なし", "無し", "サンプル", "例", "テスト",
    }
)
...
_NTA_DISCLAIMER = (
    "※国税庁の掲載情報に基づきます。税額や取扱いは文書の種類・記載内容で変わり、"
    "改正もあるため、実際の判断は原文の確認および税理士へのご相談をお願いします。"
)
...
def _is_mirasapo_url(url: str) -> bool:
    return _host_matches(url, "mirasapo-plus.go.jp")


def _is_nta_url(url: str) -> bool:
    return _host_matches(url, "nta.go.jp")


def _disclaimer_for_url(url: str) -> str:
    if _is_mirasapo_url(url):
        return _MIRASAPO_DISCLAIMER
    if _is_nta_url(url):
        return _NTA_DISCLAIMER
    return _FREEE_DISCLAIMER
...
def _fetchable_reference(url: str, allowed_hosts: set[str], handbook: Handbook) -> bool:
    """2段目で取得してよい参考URLか（許可ドメイン かつ ハンドブック記載）。"""
    from urllib.parse import urlparse

    if not url:
        return False
    host = (urlparse(url).hostname or "").lower()
    return host in allowed_hosts and _url_in_handbook(url, handbook)
