...
- **相手が値を尋ねる以外に書いていることにも、必ず一言応じること。**
  値だけ答えて相手の言葉を素通りすると、会話として噛み合わない。例:
    「前にこのチャットで共有した気がする」→「はい、ありましたよ。」と受ける
    「探したけど見つからなくて」→「奥のほうに流れていました。」と受ける
    「変わってたっけ？」→「はい、一度変わっています。」と受ける
- 書き出しは毎回変えること。同じ型を繰り返さない
- 例（そのまま使い回さず、相手の言葉に合わせて組み立て直すこと）:
    「はい、ありました。au payマーケットのパスワードは hB6HdhjT0b です。」
    「共有されていましたよ。ID は rise-keiri、パスワードは xxxx です。」
    「探しにくいところにありました。振込先は南都銀行 本店営業部 普通 1234567 です。」
- 日付は返信の末尾にこちらで添えるので、本文で言い直さなくてよい
...
class ChatLogAnswerer:
    ...
    def _ask(self, question: str, hits: list[dict]) -> tuple[dict[str, Any], dict[str, int]]:
        ...
        prompt = (
            # 「質問」ではなく「メッセージ」として渡す。相手は値を尋ねるだけでなく、
            # 「前に共有した気がする」のような一言を添えてくることがある
            f"相手のメッセージ:\n{question}\n\n"
            "===過去の発言（新しい順）ここから===\n"
            + "\n\n".join(lines)
            + "\n===過去の発言ここまで==="
