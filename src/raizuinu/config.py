...
_DEFAULTS: dict[str, Any] = {
    ...
    "fax_watch": {
        ...
        # 差出人と表題がこれに当たれば、注文でも To と対応完了の確認を付けずに置くだけにする
        # （既に届いた発注書の確認書、毎日の定例表など）。sender・title は部分一致、
        # reason は通知に添える理由（省略可）。実例 2026-09-10: G7の「発注書 情報確認表」
        "quiet_rules": [],
        # こちら（受信側）のグループ会社の名前。差出人にこれが読まれたら宛先との取り違えなので
        # 読み直させ、要約からも「当社より」「当社へ発注」の言い回しを外す（実例 2026-09-10）
        "own_company_words": [
            "ライズクリエイション", "ライズクリエーション", "RAKUTENKEN", "楽天軒", "樂天軒", "ラクテンケン",
            "ヤマトライジング", "イノベイト", "ohirome", "ライズホールディングス",
        ],
        # 急ぎの注文（表題・要約・備考・納期・品名に words の語、またはモデルが急ぎと読んだ）は、
        # 経理のほかに extra_recipients（account_id・name）にも To を付けて知らせる。
        # 実例 2026-09-23: 「セール急ぎ分」の発注看板を楽天軒の森さん・藤田さんにも
        "urgent": {
            "words": ["急ぎ", "至急", "早急", "大至急", "緊急", "特急", "急いで", "急ぐ", "ASAP"],
            "extra_recipients": [],
        },
        # 一度に複数件知らせるとき（時間外・土日に控えた分が朝にまとめて出るとき）は1通に
        # まとめ、番号（①②…）を振る。長すぎれば max_chars ごとに分ける（番号は続き）
        "batch": {"enabled": True, "max_chars": 6000},
